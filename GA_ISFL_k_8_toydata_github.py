...
# toycode of Genetic algorithm to find microbiome discovery in ISFL dataset
    ...
    if(individual.count(0) != len(individual)):
        ...
        try:
            if len(y.drop_duplicates()) == 2:
                ...
                penalty_function = _penalty_weight * abs(individual.count(1) - 8)
...
# data loading
otu_table = pd.read_csv("toydata_ISFL.csv",index_col = ["patient"])
...
creator.create("MyFitnessMulti", base.Fitness, weights=(1.0,))
...
ax3.grid(False)
fig3.savefig("toydata_ISFL_auroc_cv.svg",dpi=300)
...
ax4.grid(False)
fig4.savefig("toydata_ISFL_auroc_cv.svg",dpi=300)
